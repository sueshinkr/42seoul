...
Server::Server(int port, std::string password)
  ...
  memset(&m_stdinevents, 0, sizeof(m_stdinevents));
  ...
  if ((recv_len = recv(clnt_fd, buf, 1024, 0)) < 1) {
    ...
    if (clnt.get_m_data().find("\r\n") != std::string::npos) {
      if (clnt.get_m_data().size() > 512) {
        std::cout << "Client " << clnt_fd
                  << " : !!!IRC messages SHALL NOT exceed 512 characters in "
                     "length!!!\n";
        clnt.del_m_data();
      } else
        splitLine(clnt_fd);
...
void Server::disconnectClient(int clnt_fd) {
  ...
  std::cout << "Client " << clnt_fd << " Closed\n";
  epoll_ctl((*clnt).get_m_epoll_fd(), EPOLL_CTL_DEL, clnt_fd, &get_m_events());
  del_m_fd_to_client(clnt_fd);
  close(clnt_fd);
}

void  Server::sendBot(std::string channelName, std::string station)
{
  b.get_data(station, channelName);

  m_stdinevents.events = EPOLLIN;
  m_stdinevents.data.fd = 0;
  if (epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, 0, &m_stdinevents) == -1 && errno == ENOMEM)
    Exit::Handler();
  return ;
}
...
struct epoll_event &Server::get_m_stdevents() {
  return (m_stdinevents);
}
